...
public class ContactFormValidator implements Validator {
	...
	public void validate(Object obj, Errors errors) {
		...
		if(!contactForm.getEmail().matches(EMAIL_PATTERN)) {
			errors.rejectValue("email", "", "Ingresa un email valido");
		}
		
		if(!contactForm.getPassword().trim().equals(contactForm.getConfirmPassword())) {
			errors.rejectValue("confirmPassword", "", "El password no coincide");
		}
		
		List<String> courses = contactForm.getCourses();
		
		if(courses == null || contactForm.getCourses().size()<2) {
			errors.rejectValue("courses", "", "Debes seleccionar por lo menos dos cursos");
		}
	}
}